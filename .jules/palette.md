...
## 2024-07-18 - Improve accessibility of loading and error states
**Learning:** In the preview app, visual loading spinners (`.loader-container`) and error banners (`.error-banner`) lacked appropriate ARIA roles, making these dynamic application states invisible or delayed for screen reader users.
**Action:** Always include `role="status"` and `aria-label="Loading"` for purely visual CSS loading spinners, and `role="alert"` for error messages or banners to ensure they are immediately announced by screen readers.
