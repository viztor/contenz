...
function zodToTypeString(schema: ZodTypeAny, indent = 0): string {
  ...
  if (typeName === "enum" || typeName === "ZodEnum") {
    ...
    if (typeof values === "object") {
      return Object.keys(values).map((v) => `"${v}"`).join(" | ");
    }
    return "string";
  }
  ...
  if (typeName === "object" || typeName === "ZodObject" || (def && def.shape)) {
    ...
    const lines = entries.map(([key, value]) => {
      ...
      const isOptional = valueTypeName === "optional" || valueTypeName === "ZodOptional" || 
                         valueTypeName === "default" || valueTypeName === "ZodDefault";
...
`;
  ...
  for (const { name, types, hasI18n, metaTypeName } of collections) {
    if (types && types.length > 0) {
      ...
      const baseTypeName = metaTypeName?.replace("Meta", "") ?? name.charAt(0).toUpperCase() + name.slice(1);
...
`;
  ...
  for (const { name, stats } of collections) {
    const localeCounts = allLocales
      .map((l) => (stats.locales[l] ?? 0).toString())
      .join(" | ");
  ...
  for (const { name, items, locales } of collections) {
    ...
    const missing = items.filter(
      (item) => !locales.every((locale) => item.locales[locale])
    );
  ...
  output += "\n## Relation Validation\n\n";
  output += "_Run `npm run content:lint` for detailed relation validation._\n";
...
export async function generateCollectionFile(
  ...
  return generateI18nCollectionFile(outputPath, collectionName, items, metaTypeName, locales, schema);
}
