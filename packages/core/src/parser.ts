...
const DEFAULT_ALTERNATION = DEFAULT_EXTENSIONS.map((e) =>
  e.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")
).join("|");

// ⚡ Bolt: Cache compiled regex patterns keyed by dynamic inputs to avoid repeated recompilation in hot loops (e.g., during build).
...
function extAlternation(extensions?: string[]): string {
  if (!extensions?.length) return DEFAULT_ALTERNATION;
...
export function parseFileName(
  ...
  // ⚡ Bolt: Dynamically generate cache key to prevent collision across differing configuration states.
