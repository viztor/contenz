...
// Cache for compiled RegExp objects to avoid recompilation penalty on every file parse
// Keys are formatted as `${i18nEnabled}:${extensions.join(",")}`
...
export function parseFileName(
  ...
  const cacheKey = `${i18nEnabled}:${extensions ? extensions.join(",") : "<undefined>"}`;
