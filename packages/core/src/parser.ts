...
function extAlternation(extensions?: string[]): string {
  if (!extensions || extensions.length === 0) return DEFAULT_ALT;
...
export function parseFileName(
  ...
  // Cache key is simply i18n + the alternation string, as these are the only variables in pattern building
