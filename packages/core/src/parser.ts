...
/** Pre-computed default alternation string for performance */
const DEFAULT_ALT = DEFAULT_EXTENSIONS.map((e) => e.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")).join(
  "|"
);

/**
 * Cache for compiled filename parsing RegExps to avoid constructor overhead in hot loops.
 * Keys are format `${i18nEnabled}:${alt}`
 */
