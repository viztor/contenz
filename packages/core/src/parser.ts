...
// Cache for compiled RegExp objects to avoid recompilation overhead in hot loops
const regexCache = new Map<string, RegExp>();

function getParseRegex(i18nEnabled: boolean, extensions?: string[]): RegExp {
  const extKey = extensions?.join(",") || "default";
  const cacheKey = `${i18nEnabled}:${extKey}`;
...
export function parseFileName(
  ...
  const regex = getParseRegex(i18nEnabled, extensions);
