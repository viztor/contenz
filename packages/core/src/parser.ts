...
export function parseFileName(
  ...
  // Cache compiled RegExp to avoid recreating them and calling extAlternation on every file (~6x faster)
  const extsKey = extensions?.length ? extensions.join(",") : "";
  const cacheKey = `${i18nEnabled}-${extsKey}`;
