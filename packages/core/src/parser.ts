...
/** Pre-computed alternation for default extensions to avoid recalculation in hot loop. */
const DEFAULT_EXT_ALT = DEFAULT_EXTENSIONS.map((e) =>
  e.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")
).join("|");

/** Cache for compiled filename parsing Regular Expressions to avoid recompiling in hot loops. */
...
function extAlternation(extensions?: string[]): string {
  if (!extensions || extensions.length === 0) return DEFAULT_EXT_ALT;
