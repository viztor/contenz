...
/** Pre-computed default extensions alternation. */
const DEFAULT_ALT = DEFAULT_EXTENSIONS.map((e) => e.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")).join(
  "|"
);

/** Cache for compiled filename patterns to avoid regex recompilation in hot loops. */
const patternCache = new Map<string, RegExp>();

/** BCP 47 locale pattern for regex. */
...
function extAlternation(extensions?: string[]): string {
  if (!extensions?.length) return DEFAULT_ALT;
...
export function parseFileName(
  ...
  if (!pattern) {
    ...
    if (i18nEnabled) {
      pattern = new RegExp(`^(.+)\\.(${LOCALE_PATTERN})\\.(${alt})$`);
    } else {
