...
const PARSE_FILE_NAME_REGEX_CACHE = new Map<string, RegExp>();

function getFileNameRegex(i18nEnabled: boolean, extensions?: string[]): RegExp {
  const extKey = extensions?.length ? extensions.join(",") : "default";
  const cacheKey = `${String(i18nEnabled)}-${extKey}`;

  let regex = PARSE_FILE_NAME_REGEX_CACHE.get(cacheKey);
  if (!regex) {
    ...
    PARSE_FILE_NAME_REGEX_CACHE.set(cacheKey, regex);
  }
  return regex;
}
...
export function parseFileName(
  ...
  const regex = getFileNameRegex(i18nEnabled, extensions);
