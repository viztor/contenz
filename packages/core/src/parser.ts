...
/** Pre-computed regex alternation for default extensions */
const DEFAULT_EXTENSIONS_ALT = DEFAULT_EXTENSIONS.map((e) =>
  e.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")
).join("|");

/** Cache for compiled filename parsing regular expressions */
...
function extAlternation(extensions?: string[]): string {
  if (!extensions?.length) {
    return DEFAULT_EXTENSIONS_ALT;
  }
  return extensions.map((e) => e.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")).join("|");
}

/**
 * Helper to get or compile a regular expression for file name parsing.
 * Caches expressions to avoid new RegExp() performance penalty in tight loops.
 */
function getPattern(i18nEnabled: boolean, alt: string): RegExp {
  const cacheKey = `${i18nEnabled ? "i18n" : "std"}:${alt}`;
