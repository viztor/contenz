...
export function parseLocaleFromURL(
  ...
  if (strategy === "query") {
    ...
    if (paramValue) {
      ...
      // Bolt optimization: avoiding new Set/Map creation overhead by using Array.prototype.find
      // for small, static lists like locales, which improves performance on hot paths.
...
export function negotiateLocale(
  ...
  for (const pref of preferences) {
    // Exact match
    // Bolt optimization: use Array.prototype.find instead of creating new Map on every call
    // to avoid unnecessary memory allocations while maintaining functional equivalence.
