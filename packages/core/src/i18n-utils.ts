...
export function parseLocaleFromURL(
  ...
  if (strategy === "query") {
    ...
    if (paramValue) {
      // ⚡ Bolt: Prevent redundant processing by hoisting target string transformation outside the iteration
      const paramLower = paramValue.toLowerCase();
      // ⚡ Bolt: Use built-in array find instead of instantiating a new Set/Map for each function call to reduce memory allocation
      const match = locales.find((l) => l.toLowerCase() === paramLower);
      if (match) {
        return {
          locale: match,
...
export function negotiateLocale(
  ...
  for (const pref of preferences) {
    // ⚡ Bolt: Use built-in array find instead of instantiating a new Map to avoid overhead per request
    const exactMatch = available.find((l) => l.toLowerCase() === pref.locale);
    ...
    // ⚡ Bolt: Hoist string concatenation outside of iteration callback
    const searchPrefix = `${pref.locale}-`;
    const reverseMatch = available.find((l) =>
      l.toLowerCase().startsWith(searchPrefix)
    );
