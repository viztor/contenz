...
export function parseLocaleFromURL(
  ...
  // Optimization: Avoid allocating Set/Map for small collections, use Array.prototype.find() for better performance and memory usage in this hot path.
  if (strategy === "query") {
    const paramValue = parsedUrl.searchParams.get(queryParam);
    if (paramValue) {
      const lowerParam = paramValue.toLowerCase();
      const matchedLocale = locales.find((l) => l.toLowerCase() === lowerParam);
      if (matchedLocale) {
        return {
...
export function negotiateLocale(
  ...
  for (const pref of preferences) {
    ...
    const prefixSearch = `${pref.locale}-`;
    const reversePrefixMatch = available.find((l) =>
      l.toLowerCase().startsWith(prefixSearch)
    );
