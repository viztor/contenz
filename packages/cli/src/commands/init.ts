...
async function detectInstallCommand(cwd: string): Promise<string> {
  try {
    const packageJson = JSON.parse(await fs.readFile(path.join(cwd, "package.json"), "utf-8")) as {
...
function getScaffoldFiles(options: {
  ...
  const collectionDir = path.join(options.cwd, options.contentDir, options.collection);
...
export const initCommand = defineCommand({
  ...
  args: {
    ...
    dir: {
      type: "string",
      description: 'Collection container directory to create (used as "<dir>/*")',
    ...
    i18n: {
      type: "boolean",
      description: "Scaffold starter locale files and enable i18n in contenz.config.ts",
      default: false,
    },
    force: {
      type: "boolean",
  ...
  async run({ args }) {
    ...
    if (!args.force) {
      ...
      if (conflicts.length > 0) {
        console.error("Cannot initialize contenz because these paths already exist:");
